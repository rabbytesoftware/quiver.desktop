...
describe('useArrowDetail', () => {
	...
	it('fetches arrow detail by namespace', async () => {
		const { result } = renderHook(() => useArrowDetail('ns@v1'), { wrapper: wrapper(queryClient) });
...
describe('useCollections', () => {
	...
	it('fetches collections list', async () => {
		const { result } = renderHook(() => useCollections(), { wrapper: wrapper(queryClient) });
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
...
describe('useCollectionDetail', () => {
	...
	it('fetches collection detail by namespace', async () => {
		const { result } = renderHook(() => useCollectionDetail('col/ns'), { wrapper: wrapper(queryClient) });
		await waitFor(() => expect(result.current.isSuccess).toBe(true));
