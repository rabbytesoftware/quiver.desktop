...
impl HttpClient {
    ...
    pub async fn fetch_arrows(&self) -> Result<Vec<ArrowListResponseItem>, HttpError> {
        ...
        resp.data.ok_or_else(|| HttpError::Api(resp.error.unwrap_or_default()))
    }
    ...
    pub async fn install(
        ...
        let url = format!("{}/v0/runtime/{}/install", self.base_url, urlencoded(namespace));
    ...
    pub async fn uninstall(
        ...
        let url = format!("{}/v0/runtime/{}/uninstall", self.base_url, urlencoded(namespace));
        let body = serde_json::json!({ "variables": variables });
        self.client.post(&url).json(&body).send().await?.error_for_status()?;
        Ok(())
    }
    ...
    pub async fn execute(
        ...
        let url = format!("{}/v0/runtime/{}/{}", self.base_url, urlencoded(namespace), method);
        let body = serde_json::json!({ "variables": variables });
        self.client.post(&url).json(&body).send().await?.error_for_status()?;
        Ok(())
    }
    ...
    pub async fn stop(&self, namespace: &str) -> Result<(), HttpError> {
        let url = format!("{}/v0/runtime/{}/stop", self.base_url, urlencoded(namespace));
    ...
    pub async fn follow_collection(&self, namespace: &str) -> Result<(), HttpError> {
        let url = format!("{}/v0/collection/{}/follow", self.base_url, urlencoded(namespace));
        self.client.post(&url).send().await?.error_for_status()?;
        Ok(())
    }
...
mod tests {
    ...
    fn urlencoded_replaces_slashes() {
        assert_eq!(urlencoded("github.com/user/repo"), "github.com%2Fuser%2Frepo");
    }
    ...
    fn to_arrow_list_items_expands_multiple_versions() {
        let items = vec![ArrowListResponseItem {
            ...
            name: "Arrow".into(),
            versions: vec![
                InstalledVersion { installed_ref: "v1.0.0".into(), version: "1.0.0".into(), state: ArrowState::Ready },
                InstalledVersion { installed_ref: "v2.0.0".into(), version: "2.0.0".into(), state: ArrowState::Ready },
            ],
        }];
        let result = to_arrow_list_items(items);
