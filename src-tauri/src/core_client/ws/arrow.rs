...
use crate::core_client::types::{ArrowWsMessage, ArrowListItem, Emitter};
                    ...
                    match msg {
                        Ok(Message::Text(text)) => {
                            if let Ok(parsed) = serde_json::from_str::<ArrowWsMessage>(&text.to_string()) {
                                handle_arrow_message(parsed, &http, emitter.as_ref()).await;
                            }
                        }
...
mod tests {
    use super::*;
    use crate::core_client::types::{ArrowState, CoreStatus, RuntimeUpdatePayload};
    use std::sync::Mutex;
